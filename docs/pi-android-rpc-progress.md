...
| 4.2 Sessions cache repo | DONE | PENDING | ✅ ktlintCheck, detekt, test; smoke: :core-sessions:test --tests "*SessionIndexRepositoryTest" | Implemented `core-sessions` cached index repository per host with file/in-memory cache stores, background refresh + merge semantics, and query filtering support with deterministic repository tests. |
