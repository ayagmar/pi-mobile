...
| 4.1 Host profiles + secure token | DONE | 895c163 | ✅ ktlintCheck, detekt, test | Added host profile CRUD flow (Compose hosts screen + editor dialog + persistence via SharedPreferences), plus Keystore-backed token storage via EncryptedSharedPreferences (security-crypto). |
